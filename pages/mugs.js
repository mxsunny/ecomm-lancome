...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in mugss) {
      if (!mugss[item.title].color.includes(item.color) && item.availableQty > 0) {
        mugss[item.title].color.push(item.color);
      }
      if (!mugss[item.title].size.includes(item.size) && item.availableQty > 0) {
        mugss[item.title].size.push(item.size);
      }
    } else {
      mugss[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availableQty > 0) {
        mugss[item.title].color = [item.color]
        mugss[item.title].size = [item.size]
      }
    }
  }

  return {
    props: { products: JSON.parse(JSON.stringify(mugss)) },
  }
}
