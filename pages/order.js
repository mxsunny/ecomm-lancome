            ...
            <div className="lg:w-1/2 w-full lg:pr-10 lg:py-6 mb-6 lg:mb-0">
              ...
              {Object.keys(product).map((key) => {
                return <div key={key} className="flex border-t border-gray-200 py-2">
                  <span className="text-gray-500">{product[key].name} ({product[key].size}/{product[key].variant})</span>
                  <span className="ml-auto text-gray-900"> {product[key].qty}</span>
                  <span className="ml-auto text-gray-900">₹{product[key].price}*{product[key].qty}=₹{product[key].price * product[key].qty}</span>
                </div>
              })}
